...
test.describe('Employee CRUD', () => {
  ...
  test('added employee row shows correct name, email and position', async ({ page }) => {
    const listPage = new EmployeeListPage(page);
    const formPage = new EmployeeFormPage(page);
  ...
  test('edit dialog is pre-populated with existing employee data', async ({ page }) => {
  ...
  test('submitting empty add form does not add employee (HTML5 validation)', async ({ page }) => {
    const listPage = new EmployeeListPage(page);
    await listPage.clickAddEmployee();

    // Click submit without filling fields — browser required validation blocks submission
  ...
  test('view dialog shows all employee details', async ({ page }) => {
