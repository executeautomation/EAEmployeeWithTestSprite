...
export class EmployeeListPage {
  ...
  /** Wait for the employee table body to be visible. */
  async waitForList() {
  ...
  /**
   * Locator for an employee name cell in the table.
  ...
  /** Returns 0 when the "No employees found." row is displayed. */
  ...
  /** Click the Edit button in the row that contains the given employee name. */
  async clickEditFor(name: string) {
    const row = this.page.locator('tbody tr').filter({ hasText: name });
    await row.getByRole('button', { name: 'Edit' }).click();
  }

  /** Click the Delete button in the row that contains the given employee name. */
